<script lang="ts">
	...
	export let showInvertedLabel: boolean =
		labelAlignX == "center" && labelAlignY == "middle"; //top, bottom
...
{#if showInvertedLabel}
	<foreignObject
		class="progress-value progress-value-{style} progress-value-inverted"
		x="0"
		y="0"
		width="100%"
		height="100%">
		<div
			class="progress-value-content {labelAlignX} {labelAlignY}"
			style={invLblStyle.join(";")}>
			{@html $store.label}
		</div>
	</foreignObject>
{/if}
...
<g mask={showInvertedLabel ? "url(#" + maskId + ")" : null}>
	<foreignObject
		class="progress-value progress-value-{style}"
		x="0"
		y="0"
		width="100%"
		height="100%">
